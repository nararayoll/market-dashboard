...
from portfolio_analysis import fetch_portfolio, get_signals, fetch_news, fetch_portfolio_history
...
def build_dashboard():
    if not os.path.exists(DATA_FILE):
        print("market_data.csv 없음. market_summary.py 를 먼저 실행하세요.")
        return
    ...
    chart_datasets = {name: {"dates": d["dates"], "values": d["values"]} for name, d in market_hist.items()}
...
<div style="display:flex;align-items:center;gap:16px;margin:32px 0 12px;">
  ...
  <div style="display:flex;gap:12px;align-items:center;font-size:0.75rem;">
    <span style="display:flex;align-items:center;gap:5px;">
      <span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:#4caf50;"></span>
      <span style="color:#888;">긍정적 뉴스 (상승·호재)</span>
    </span>
    <span style="display:flex;align-items:center;gap:5px;">
      <span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:#f44336;"></span>
      <span style="color:#888;">부정적 뉴스 (하락·악재)</span>
    </span>
    <span style="display:flex;align-items:center;gap:5px;">
      <span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:#444;"></span>
      <span style="color:#888;">중립</span>
    </span>
  </div>
</div>
...
Object.entries(groups).forEach(([groupName, tickers]) => {{
  ...
  validTickers.forEach(name => {{
    const canvasId = 'chart_' + name.replace(/[^a-zA-Z0-9]/g, '_');
    box.innerHTML += `<canvas id="${{canvasId}}" height="120"></canvas>`;
  }});
  ...
  validTickers.forEach(name => {{
    ...
    const chartLabels = d.dates || labels;
    const chartValues = d.values !== undefined ? d.values : d;
    new Chart(ctx, {{
      type: 'line',
      data: {{
        labels: chartLabels,
        datasets: [{{ label: name, data: chartValues, borderColor: color, backgroundColor: color + '22',
...
if (newsList.length === 0) {{
  ...
  const grouped = {{}};
  newsList.forEach(n => {{
    const key = n.stock + '|' + n.ticker;
    if (!grouped[key]) grouped[key] = {{ stock: n.stock, ticker: n.ticker, articles: [] }};
    grouped[key].articles.push(n);
  }});
    ...
    g.articles.forEach(a => {{
      ...
      articlesHtml += `
        <div class="news-item ${{cls}}">
          <span class="news-sentence">${{a.title_ko}}</span>
          <div class="news-item-meta">
            ${{a.publisher}} · ${{a.date}}
            <a class="news-link" href="${{a.link}}" target="_blank">원문 →</a>
          </div>
        </div>`;
    }});
    grid.innerHTML += `
      <div class="news-card">
        <div class="stock-tag">${{g.stock}} <span style="color:#555">(${{g.ticker}})</span></div>
...
Object.entries(portHistory).forEach(([ticker, d]) => {{
  const box = document.createElement('div');
  box.className = 'port-chart-box';
  const cur = d.currency === 'KRW' ? '₩' : '$';
  const canvasId = 'pchart_' + ticker.replace(/[^a-zA-Z0-9]/g, '_');
  box.innerHTML = `
    <h3>${{d.name}} <span style="color:#555;font-size:.72rem">${{ticker}}</span></h3>
    <div class="sub">종가 · MA20 · MA60 · 매입가 ${{cur}}${{d.avg_price.toLocaleString()}}</div>
  ...
  new Chart(ctx, {{
    ...
    data: {{
      labels: d.dates,
      datasets: [
        {{ label: '종가', data: d.prices, borderColor: '#4fc3f7', backgroundColor: '#4fc3f722',
           borderWidth: 2, pointRadius: 0, fill: true, tension: 0.2, spanGaps: true, order: 1 }},
        {{ label: 'MA20', data: d.ma20, borderColor: '#ffb74d', borderWidth: 1.5,
    ...
    options: {{
      ...
      interaction: {{ intersect: false, mode: 'index' }},
      plugins: {{
        legend: {{ labels: {{ color: '#777', font: {{ size: 10 }}, boxWidth: 20, padding: 10 }} }},
        tooltip: {{ backgroundColor: '#2a2a3a', titleColor: '#fff', bodyColor: '#ccc' }}
      }},
      scales: {{
...
if (portSignals.length === 0) {{
  ...
  portSignals.forEach(s => {{
    const el = document.createElement('div');
    el.className = `alert ${{s.type === 'buy' ? 'safe' : 'danger'}}`;
    el.textContent = (s.type === 'buy' ? '▲ 매수 | ' : '▼ 매도 | ') + s.msg;
    portSigEl.appendChild(el);
  }});
}}
...
portfolio.forEach(r => {{
  ...
  const maStatus = r.ma60
    ? (r.ma20 > r.ma60 ? '<span style="color:#4caf50">MA20 > MA60</span>' : '<span style="color:#f44336">MA20 < MA60</span>')
    : 'N/A';
  let sigCell = '<span style="color:#444">-</span>';
  if (r.cross === 'golden' || r.rsi_signal === 'oversold')
    sigCell = '<span style="color:#4caf50;font-weight:700">▲ 매수 고려</span>';
  else if (r.cross === 'dead' || r.rsi_signal === 'overbought')
    sigCell = '<span style="color:#f44336;font-weight:700">▼ 매도 고려</span>';
  tbody.innerHTML += `
    <tr>
